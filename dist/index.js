    ...
    return {
        ...
        setType: function(any, type) {
            if (typeof(any) === "object") {
                if (Object.prototype.toString.call(any) === '[object Array]') {
                    switch (type) {
                        ...
                        case "number":
                            return any.length;
                        case "object":
                            return any.reduce(function(p, c, i) {
                                p[i] = c;
                                return p;
                            }, {});
                        case "array":
                    ...
                    switch (type) {
                        ...
                        case "string":
                            return Object.entries(any).reduce(function(p, [k, v]) {
                                return p + k + "\=" + v + "\;";
                            }, "");
                        case "object":
                            return any;
                ...
                switch (type) {
                    case "boolean":
                        return (any === "true" || any === "false") ? any === "true" : any !== "";
                    case "string":
                        return any;
                    case "array":
                        return [any];
                    case "regexp":
                        return new RegExp(any.replace(/^\/|\/[dgimsuy]{0,7}$/g, ""), any.replace(/.*\/([dgimsuy]{0,7})$/, "$1"));
                }
            } else if (typeof(any) === "number") {
                if (!isNaN(any) && isFinite(any)) {
                    switch (type) {
        ...
        getOs: function() {
            var clients = [{
                ...
                {
                    system: 'Mac OS',
                    pattern: /(Mac OS|MacPPC|MacIntel|Mac_PowerPC|Macintosh)/
                },
                {
                    system: 'QNX',
                    pattern: /QNX/
                ...
                {
                    system: 'Search Bot',
                    pattern: /(nuhk|Googlebot|Yammybot|Openbot|Slurp|MSNBot|Ask Jeeves\/Teoma|ia_archiver)/
                }
            ];
        ...
        compare2: function(a, b) {
            ...
            while (i < l) {
                ...
                if (type1 === "number") {
                    ...
                    if (str1.localeCompare(str2, "en", {
                            sensitivity: "base"
                        }) > 0) {
                        return 1;
                    }
                    if (str2.localeCompare(str1, "en", {
                            sensitivity: "base"
                        }) > 0) {
                        return -1;
                    }
        ...
        sortArray: function(arr, descending, caseSensitive) {
            ...
            return arr.slice(0).sort(function(a, b) {
                var aa = typeof(a) === "number" ? [a] : (CASE_SENSITIVE ? a.split(/(\d+)/) : a.toLowerCase().split(/(\d+)/));
                var bb = typeof(b) === "number" ? [b] : (CASE_SENSITIVE ? b.split(/(\d+)/) : b.toLowerCase().split(/(\d+)/));
                var i = 0;
        ...
        path: {
            ...
            parse: function() {
                ...
                isFile = /[^\/]+\.[0-9A-Za-z]{1,}$/.test(str);
                    
                fileName = str.split("\/").pop();

                if (isFile) {
                    extension = fileName.split("\.").pop();
                    baseName = fileName.replace(/\.[0-9A-Za-z]{1,}$/, "");
        ...
        execQuery: function(data, query) {
            ...
            var calc = function(a, b, operator) {
                switch (operator) {
                    ...
                    case "$not":
                        return OPERATORS.$not(a, b);
                    case "$eq":
                        return OPERATORS.$eq(a, b); // equal
                    case "$ne":
                        return OPERATORS.$ne(a, b); // not equal
                    case "$in":
                        return OPERATORS.$in(a, b); // include
                    case "$nin":
                        return OPERATORS.$nin(a, b); // exclude
                    case "$gt":
                        return OPERATORS.$gt(a, b); // greater than
                    case "$gte":
                        return OPERATORS.$gte(a, b); // greater than or equal
                    case "$lt":
                        return OPERATORS.$lt(a, b); // less than or equal
                    case "$lte":
                        return OPERATORS.$lte(a, b); // less than or equal
                    case "$exists":
                        return OPERATORS.$exists(a, b);
                    case "$regexp":
                        return OPERATORS.$regexp(a, b); // RegExp.test()
                    default:
                        var err = new Error('invalid argument type');
                        err.name = "TypeError";
                        throw err;
